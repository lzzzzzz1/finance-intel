...
网站详情页会先展示“AI速读”：一句话摘要、可能影响、情绪判断、风险提示、关键数字和注意事项。你可以先在站内看提炼结果；点击“官网原文”才会跳到公告或新闻的官方来源页面。

- 配置了 `OPENAI_API_KEY`：GitHub Actions 定时更新时会调用 OpenAI 做中文摘要、风险和量化打分。
...
- [Trafilatura](https://github.com/adbar/trafilatura)：适合参考网页正文抽取、元数据识别和去除导航/页脚噪声的思路。
- [edgartools](https://github.com/dgunning/edgartools)：适合后续增强 SEC EDGAR 财报结构化解析。

当前优化先采用低依赖方案：板块页改为研究工作台式布局，事件详情增加“AI速读”“站内阅读”“证据摘录”“新手解释”和“关键数字怎么读”。后端参考 Trafilatura/readability 类项目的正文优先思路，增加了轻量 HTML 正文抽取、阅读分段和证据片段定位，避免只显示孤立标题或数字。
